...
public class BalanceServiceTest {
    ...
    public void setUp(){
        ...
        balanceServiceMock = new BalanceService(accountRepositoryMock);
    }

    @Test
    public void testWithdrawal(){
        when(accountRepositoryMock.findById(1L)).thenReturn(createAccount());
        final BalanceWrapperDto withdrawalResponse = balanceServiceMock.getBalance(1L);
        final BalanceDto balance = withdrawalResponse.getBalance();

        Assertions.assertThat(balance.getAmount()).isEqualTo(300);
    }
